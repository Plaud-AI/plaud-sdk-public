...
final class RecordingViewController: UIViewController {
    ...
    /// Fallback that restores the record button if the device never acknowledges the start.
    private var startAckTimer: Timer?
    ...
    /// Transitional "waiting for the device" state after the user taps record. Cleared by
    /// updateUI(for:) on the next real state, or by the timeout if the device never answers.
    private func showStartingFeedback() {
        subtitleLabel.text = "Starting..."
        recordButton.isUserInteractionEnabled = false
        recordButton.alpha = 0.5
        startAckTimer?.invalidate()
        startAckTimer = Timer.scheduledTimer(withTimeInterval: 4.0, repeats: false) { [weak self] _ in
            guard let self, case .idle = self.currentState else { return }
    ...
    @objc private func recordTapped() {
        ...
        case .idle:
            ...
            // updateUI(for:) overwrites this the moment the real state arrives.
