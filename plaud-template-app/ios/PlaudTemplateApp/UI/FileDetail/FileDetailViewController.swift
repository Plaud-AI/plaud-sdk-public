...
final class FileDetailViewController: UIViewController {
    ...
    private func loadContent() {
        // Prefer the persisted record: updateTranscript/updateDuration write to disk, but the
        // file list we were constructed from (SyncManager's in-memory snapshot) can be stale and
        // still carry a nil transcriptJSON — reading the store first mirrors Android's loadFile.
        ...
        // Show the player whenever the synced audio exists — playback must not depend on
        // the transcript (mirrors Android; prepareAudioPlayer no-ops if the file is missing).
        prepareAudioPlayer()
    ...
    private func updateTabContent() {
        if selectedTab == 0 {
            ...
            if !transcriptResults.isEmpty {
                ...
                generateButton.isHidden = false
                // Player visibility is owned by prepareAudioPlayer() (audio presence), not tabs
                progressLabel.isHidden = true
                progressSubLabel.isHidden = true
            }
        }
    ...
    /// "Speaker N  ·  HH:MM:SS" header for one segment. Shared by the on-screen rendering and
    /// Copy Transcript so the copied text can't drift from what the user sees (PLA2-394).
    private func transcriptHeader(for r: TranscriptionResult) -> String {
        let time = formatSeconds(r.start ?? 0)
        let speaker = r.speakerId?.replacingOccurrences(of: "SPEAKER_", with: "Speaker ") ?? "Speaker"
        return "\(speaker)  ·  \(time)"
    }

    /// Copy payload: speaker + timestamp header above each segment, blank line between segments.
    /// Plain text alone was useless for anyone trying to find the moment in the recording.
    private func transcriptPlainText() -> String {
        transcriptResults
            .map { "\(transcriptHeader(for: $0))\n\($0.text ?? "")" }
            .joined(separator: "\n\n")
    }
    ...
    private func showTranscriptResults(_ results: [TranscriptionResult]) {
        ...
        for r in results {
            ...
            headerLabel.text = transcriptHeader(for: r)
    ...
    @objc private func showMoreActions() {
        ...
        if !transcriptResults.isEmpty {
            sheet.addAction(UIAlertAction(title: "Copy Transcript", style: .default) { [weak self] _ in
                UIPasteboard.general.string = self?.transcriptPlainText() ?? ""
            })
        }
