...
final class ScanningViewController: UIViewController {
    ...
    /// SNs on screen when the user dismissed the sheet — see the scannedDevicesPublisher sink.
    private var dismissedDeviceSNs = Set<String>()
    /// Latest scan result, kept so onDismiss knows what the user was looking at.
    private var latestScannedSNs = Set<String>()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if isAddingDevice {
            ...
            // BLE needs a moment to actually drop the old link; scanning inside that window finds
            // nothing, which looked like the connect sheet never opening (PLA2-331).
    ...
    private func setupBindings() {
        ...
        deviceManager.scannedDevicesPublisher
            ...
            .sink { [weak self] devices in
                ...
                // Scanning keeps emitting the same devices, so re-presenting on every emission put
                // the sheet straight back after the user closed it (PLA2-330). Only a device the
                // user hasn't already dismissed is new information worth interrupting for.
                let sns = Set(devices.map { $0.serialNumber })
                self.latestScannedSNs = sns
                guard !sns.isSubset(of: self.dismissedDeviceSNs) else { return }
        ...
        deviceManager.connectionStatePublisher
            ...
            .sink { [weak self] state in
                ...
                case .disconnected:
                    ...
                    self.scheduleRescan()
                case .failed(let message) where message == DeviceManager.bluetoothOffMessage:
                    // Bluetooth off is the one failure the user can fix right now, and the spinning
                    // animation hides the fact that nothing is happening (PLA2-321).
                    self.presentBluetoothOffAlert(message)
                default:
                    break
                }
            }
    ...
    /// Bluetooth is off: say so and hand the user the Settings app. Re-armed on dismiss so a second
    /// scan attempt can prompt again, but never stacks two alerts.
    private func presentBluetoothOffAlert(_ message: String) {
        guard !isShowingBluetoothAlert, presentedViewController == nil else { return }
        isShowingBluetoothAlert = true
        statusLabel.text = message
        let alert = UIAlertController(title: "Bluetooth Is Off", message: message, preferredStyle: .alert)
    ...
    private func showConnectSheet() {
        ...
        sheet.onDismiss = { [weak self] in
            ...
            self.dismissedDeviceSNs = self.latestScannedSNs
        }
