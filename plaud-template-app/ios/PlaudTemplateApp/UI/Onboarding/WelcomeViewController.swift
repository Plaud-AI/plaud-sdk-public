...
final class WelcomeViewController: UIViewController {
    ...
    /// Enter without a device (connect later from Home)
    private lazy var connectLaterButton: UIButton = {
        let btn = UIButton(type: .system)
        btn.setTitle("Connect device later", for: .normal)
        btn.setTitleColor(UIColor(hex: "#7A7A7A"), for: .normal)
        btn.titleLabel?.font = .systemFont(ofSize: 14)
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.addTarget(self, action: #selector(connectLaterTapped), for: .touchUpInside)
        return btn
    }()
    ...
    private func setupLayout() {
        [backgroundImageView, logoContainer, appNameLabel, getStartedButton, connectLaterButton].forEach { view.addSubview($0) }
        ...
        NSLayoutConstraint.activate([
            ...
            // Get Started button, above the connect-later entry
            getStartedButton.bottomAnchor.constraint(equalTo: connectLaterButton.topAnchor, constant: -8),
            ...
            // Connect-later text entry, 16pt from bottom safeArea
            connectLaterButton.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -16),
            connectLaterButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            connectLaterButton.heightAnchor.constraint(equalToConstant: 40),
        ])
    }
    ...
    @objc private func connectLaterTapped() {
        let userId = Self.extractUserId()
        RecordingStore.shared.userId = userId
        RecordingStore.shared.hasSkippedOnboarding = true
        DeviceManager.shared.configure(userId: userId)
