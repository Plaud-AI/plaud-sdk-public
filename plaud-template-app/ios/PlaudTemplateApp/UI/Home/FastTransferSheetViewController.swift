...
final class FastTransferSheetViewController: UIViewController {
    ...
    /// Persisted "Never show again" preference; read by the Home entry point before presenting.
    static let neverShowAgainKey = "fastTransfer_neverShowAgain"
    ...
    @objc private func turnOnTapped() {
        // Denied location can't be re-prompted by iOS — tapping again would do nothing, so
        // surface an actionable alert and close the sheet (PLA2-392).
    ...
    @objc private func checkboxTapped() {
        ...
        if checkboxButton.isSelected {
            UserDefaults.standard.set(true, forKey: Self.neverShowAgainKey)
        } else {
            UserDefaults.standard.removeObject(forKey: Self.neverShowAgainKey)
        }
    }
}
