...
final class HomeViewController: UIViewController {
    ...
    private func showFastTransferDialog() {
        // Location access is required by NEHotspotConfiguration. Once denied, iOS never shows the
        // system prompt again — offer a jump to Settings instead of failing silently.
        if syncManager.isLocationPermissionDenied {
            presentLocationPermissionAlert()
            return
        }
        // Honor "Never show again": skip the sheet and start directly (mirrors Android).
        if UserDefaults.standard.bool(forKey: FastTransferSheetViewController.neverShowAgainKey) {
    ...
    private func presentLocationPermissionAlert() {
        let alert = UIAlertController(
            ...
            message: "WiFi fast transfer needs location access to join the device hotspot. Enable it in Settings > Privacy > Location Services.",
...
final class DeviceCardView: UIView, UIGestureRecognizerDelegate {
    ...
    func configure(with device: PlaudDevice?) {
        ...
        // Battery (color: >20% green, <=20% orange, <=10% red). A negative level means "not
        // reported yet" (the SDK hands us -1 until the device sends it, which showed up as "-1%" on
        // NotePin S — PLA2-312): show a placeholder and a neutral bar instead of a nonsense number.
        let level = device.batteryLevel
        let chargingPrefix = device.isCharging ? "⚡ " : ""
        batteryValueLabel.text = level >= 0 ? "\(chargingPrefix)\(level)%" : "\(chargingPrefix)--"
        if level < 0 {
            batteryFill.backgroundColor = UIColor(hex: "#A3A3A3")
        } else if level <= 10 {
    ...
    private func updateBars() {
        ...
        let batteryRatio = CGFloat(max(cachedDevice?.batteryLevel ?? 0, 0)) / 100
...
final class SyncBannerView: UIView {
    ...
    func configure(progress: SyncProgress, isWiFi: Bool) {
        if progress.totalFiles > 0 {
            ...
            // Download phase: percentage (byte progress) + speed when available. Post-download the
            // SDK reports the local transcode/write at 100% — show "Converting…" with no moving
            // number, since this is processing, not transfer.
            if progress.isConverting {
                speedLabel.text = "Converting…"
            } else {
                let percent = Int((ratio * 100).rounded())
                speedLabel.text = progress.speedText.isEmpty ? "\(percent)%" : "\(percent)% · \(progress.speedText)"
            }
        } else {
            countLabel.text = ""
            progressFillWidth?.isActive = false
