...
final class PlaudAPIService {
    ...
    private var baseURL: String { "https://\(RecordingStore.shared.activeServerDomain)/developer/api" }
    ...
    /// claim so it always matches the account/environment in use; Info.plist value as fallback.
    private var clientId: String {
        if let claim = JwtUtils.parse(userAccessToken)?.clientId, !claim.isEmpty {
            return claim
        }
        return Bundle.main.object(forInfoDictionaryKey: "PlaudClientId") as? String ?? ""
    }

    /// Transcription API key. Runtime override (Settings → API Key) takes precedence;
    /// read per-request, so replacement applies immediately.
    var apiKey: String {
        if let override = UserDefaults.standard.string(forKey: "apiKeyOverride"), !override.isEmpty {
            return override
        }
        return Bundle.main.object(forInfoDictionaryKey: "PlaudApiKey") as? String ?? ""
    }

    /// User Access Token (for SDK endpoints and file upload).
    /// Runtime override (Settings → Replace) takes precedence, matching DeviceManager.
    ...
    func uploadPartToS3(
        ...
        #if DEBUG
        AppLog.log("[PlaudAPI] >>> PUT S3 part (\(data.count) bytes)")
        #endif
        session.dataTask(with: request) { _, response, error in
            if let error = error {
                #if DEBUG
                AppLog.log("[PlaudAPI] <<< S3 PUT error: \(error.localizedDescription)")
                #endif
            ...
            #if DEBUG
            AppLog.log("[PlaudAPI] <<< S3 PUT OK, ETag=\(etag)")
            #endif
    ...
    // MARK: - Device Cloud Binding (lifecycle guide §3.1/§3.2)

    /// POST /open/partner/sdk/bind | unbind with the user access token.
    /// Completion returns the HTTP status code, or nil on transport failure.
    func reportDeviceBinding(action: String, type: String, sn: String, completion: @escaping (Int?) -> Void) {
        guard let url = URL(string: "\(baseURL)/open/partner/sdk/\(action)") else { completion(nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("Bearer \(userAccessToken)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try? JSONSerialization.data(withJSONObject: ["type": type, "sn": sn])
        AppLog.log("[PlaudAPI] cloud \(action) >>> POST \(url.absoluteString) body={\"type\":\"\(type)\",\"sn\":\"\(sn)\"} Authorization=Bearer \(userAccessToken.prefix(16))…")

        session.dataTask(with: request) { data, response, error in
            if let error = error {
                AppLog.log("[PlaudAPI] cloud \(action) <<< transport error: \(error.localizedDescription)")
                completion(nil)
                return
            }
            let status = (response as? HTTPURLResponse)?.statusCode ?? 0
            let body = data.flatMap { String(data: $0, encoding: .utf8) } ?? ""
            AppLog.log("[PlaudAPI] cloud \(action) <<< HTTP \(status) \(body.prefix(300))")
            completion(status)
        }.resume()
    }
    ...
    private func perform<T: Decodable>(_ request: URLRequest, completion: @escaping (Result<T, Error>) -> Void) {
        ...
        #if DEBUG
        if !isS3 {
            AppLog.log("[PlaudAPI] >>> \(method) \(urlStr)")
        }
        #endif
        ...
        session.dataTask(with: request) { data, response, error in
            if let error = error {
                #if DEBUG
                AppLog.log("[PlaudAPI] <<< NETWORK ERROR: \(error.localizedDescription)")
                #endif
                completion(.failure(error))
                return
            }
            ...
            guard (200...299).contains(statusCode) else {
                ...
                #if DEBUG
                AppLog.log("[PlaudAPI] <<< HTTP \(statusCode): \(body)")
                #endif
            ...
            do {
                ...
                if T.self == EmptyResponse.self, let empty = EmptyResponse() as? T {
                    ...
                    #if DEBUG
                    AppLog.log("[PlaudAPI] <<< Decode error: \(error)")
                    #endif
                    completion(.failure(error))
                }
            }
