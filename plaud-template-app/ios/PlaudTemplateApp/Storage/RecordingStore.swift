...
final class RecordingStore {
    ...
    var hasSkippedOnboarding: Bool {
        get { UserDefaults.standard.bool(forKey: "hasSkippedOnboarding") }
        set { UserDefaults.standard.set(newValue, forKey: "hasSkippedOnboarding") }
    }

    // MARK: - Server Environment (domain switch, restart to apply)

    static let prodServerDomain = "platform-us.plaud.ai"
    static let preServerDomain = "platform-us-pre.plaud.ai"
    static let testServerDomain = "platform-test.plaud.ai"
    ...
    static let serverEnvironments: [(label: String, domain: String)] = [
        ("Production", prodServerDomain),
        ("Pre-release", preServerDomain),
        ("Test", testServerDomain),
    ]

    /// Display label for a domain ("Custom" if it isn't one of the known environments).
    static func serverEnvironmentLabel(for domain: String) -> String {
        serverEnvironments.first { $0.domain == domain }?.label ?? "Custom"
    }

    /// Runtime-selected server domain (Settings → Environment). Restart to take effect.
    var serverDomainOverride: String? {
        get { UserDefaults.standard.string(forKey: "serverDomainOverride") }
